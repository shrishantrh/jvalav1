import { BarChart3, Brain, TrendingUp, Sparkles, Target, Clock, Utensils, Pill, Heart, Shield, Activity, Thermometer, Moon, Zap, MapPin, CalendarDays, Scale, Flame, Dumbbell, Droplets, Eye, Wind, Beaker, AlertTriangle, Leaf, Timer, Layers, Gauge, Footprints, Coffee, Stethoscope, Waves, ScanLine, GitCompare } from "lucide-react";
...
// 35 capability buttons — each maps to a real analytical feature
const CAPABILITY_PROMPTS = [
  ...
  { icon: Scale, label: "Health score", prompt: "What's my current health score and what factors are bringing it down? Give me the full breakdown including escalation penalty, meal regularity, and the single change that would improve it the most.", color: "text-amber-500" },
  ...
  { icon: Activity, label: "Severity trajectory", prompt: "Is my condition getting better or worse over time? Show me my EWMA severity trend and tell me honestly where things are heading.", color: "text-rose-500" },
  ...
  { icon: Leaf, label: "Diet deep-dive", prompt: "Break down my diet: daily calorie averages, protein/fiber/sugar/sodium ratios, meal timing regularity, late-night eating rate, caffeine patterns, alcohol intake, and which specific foods are suspicious vs protective.", color: "text-green-600" },
  ...
  { icon: Gauge, label: "Med adherence", prompt: "How consistent am I with my medications? Show me adherence rates for each medication, timing consistency scores, gap days, and whether adherence correlates with fewer flares.", color: "text-purple-400" },
  ...
  { icon: Stethoscope, label: "Escalation detection", prompt: "Am I in an escalation pattern? How many rapid escalation windows have I had? What's my EWMA trend showing? Should I be concerned?", color: "text-rose-600" },
...
export const generateFollowUps = (aiResponse: string, hasCharts: boolean): string[] => {
  ...
  if (lower.includes('medication') || lower.includes('medicine') || lower.includes('drug')) {
    ...
    followUps.push("How many medication gap days do I have?");
  }
  ...
  if (lower.includes('food') || lower.includes('diet') || lower.includes('eat') || lower.includes('calori')) {
    ...
    followUps.push("How regular are my meal times?");
    followUps.push("How's my caffeine and alcohol intake?");
  }
  
  if (lower.includes('severity') || lower.includes('worse') || lower.includes('better') || lower.includes('trajectory')) {
    followUps.push("Show my EWMA severity trend");
  ...
  if (lower.includes('escalat') || lower.includes('acceleration')) {
